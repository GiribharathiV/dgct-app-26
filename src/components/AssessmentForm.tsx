...
const AssessmentForm: React.FC<AssessmentFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const updateQuestion = (index: number, field: string, value: any) => {
    ...
    // Handle special case for updating question type
    if (field === 'type') {
      if (value === 'multiple-choice') {
        ...
        // If changing from multiple-choice to text, remove options and correctAnswer
  ...
  const updateOption = (questionIndex: number, optionIndex: number, value: string) => {
    ...
    if (newQuestions[questionIndex].options) {
      ...
      // If the correct answer was updated, update it
  ...
  const removeOption = (questionIndex: number, optionIndex: number) => {
    ...
    if (newQuestions[questionIndex].options && newQuestions[questionIndex].options?.length > 2) {
      ...
      // If removing the correct answer, set a new one
      if (newQuestions[questionIndex].correctAnswer === questions[questionIndex].options?.[optionIndex]) {
        // Find a different option to be the correct answer
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === 'multiple-choice') {
        ...
        // Check if correctAnswer is set and is one of the options
